...
class EmailService {
    constructor(apiKey, companyService) {
        this.companyService = companyService;
        this.resend = new resend_1.Resend(apiKey);
    }
    sendEmail(_a) {
        return __awaiter(this, arguments, void 0, function* ({ to, subject, html, }) {
            const company = yield this.companyService.getCompany();
            if (!company) {
                throw new errors_1.NotFoundError("Company not found. Call loadCompany() first.");
            }
            const mailOptions = {
            ...
            yield this.resend.emails.send(mailOptions);
        });
    }
    sendResetPasswordEmail(_a) {
        return __awaiter(this, arguments, void 0, function* ({ name, email, token, origin, expirationHours = 1, }) {
            const company = yield this.companyService.getCompany();
            if (!company) {
                throw new errors_1.NotFoundError("Company not found. Call loadCompany() first.");
            }
    ...
    <div style="margin-bottom: 32px; text-align: center;">
      ...
      <div style="margin-top: 32px; color: #4b5563; font-size: 14px;">
        <p>This link will expire in ${expirationHours} hour${expirationHours !== 1 ? "s" : ""} for security reasons.</p>
    ...
    <div style="border-top: 1px solid #e5e7eb; padding-top: 24px; text-align: center;">
      <div style="color: #6b7280; font-size: 14px;">
        <p>${company.name}</p>
        <p style="margin-top: 8px;">Need help? <a href="mailto:${company.email}" style="color: #9333ea; text-decoration: underline;">Contact support</a></p>
      </div>
    </div>
  </div>`;
    ...
    sendVerificationEmail(_a) {
        return __awaiter(this, arguments, void 0, function* ({ name, email, verificationToken, origin, expirationHours = 24, }) {
            const company = yield this.companyService.getCompany();
            if (!company) {
                throw new errors_1.NotFoundError("Company not found. Call loadCompany() first.");
            }
    ...
    sendWelcomeEmail(_a) {
      ...
      <div style="background: linear-gradient(135deg, #9333ea 0%, #4f46e5 100%); padding: 32px; text-align: center;">
        <h1 style="font-size: 28px; font-weight: 700; color: white; margin-bottom: 8px;">Welcome to ${company.name}!</h1>
      ...
      <div style="padding: 24px;">
        ...
        <div style="border-top: 1px solid #e5e7eb; padding-top: 24px;">
          ...
          <a href="mailto:${company.email}" style="color: #9333ea; text-decoration: none; font-weight: 500;">Contact Support</a>
        </div>
      </div>
      ...
      <div style="background-color: #f9fafb; padding: 24px; text-align: center; font-size: 12px; color: #6b7280;">
        <p style="margin-bottom: 16px;">
          You're receiving this email because you signed up for an account on ${company.name}.
        </p>
        <p style="margin-bottom: 16px;">
